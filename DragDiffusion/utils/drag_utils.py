...
def drag_diffusion_update(model,
    ...
    if optimize_text:
        target_embeddings = copy.deepcopy(text_embeddings)
        target_embeddings.requires_grad_(True)
        text_optimizer = torch.optim.Adam([target_embeddings], lr=args.text_lr)
    else:
    ...
    for step_idx in range(args.n_pix_step):
        with torch.autocast(device_type='cuda', dtype=torch.float16):
            ...
            for i in range(len(handle_points)):
                ...
                if optimize_text:
                    f1_cur_patch = f1[:,:,r1:r2, c1:c2].detach()
                    f1_next_patch = interpolate_feature_patch(f1,r1+di[0],r2+di[0],c1+di[1],c2+di[1])
                    loss_text += ((2*args.r_m+1)**2)*F.l1_loss(f1_cur_patch, f1_next_patch)
                    if args.text_mask:
                        loss_text += args.text_lam * (text_embeddings[:,:args.pad_idx,:]-target_embeddings[:,:args.pad_idx,:]).abs().sum()
    ...
    return init_code, text_embeddings, target_embeddings.detach().requires_grad_(False)

# ! Currently, DragText does not support it.
