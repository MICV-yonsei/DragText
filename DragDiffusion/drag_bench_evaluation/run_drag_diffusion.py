...
def run_drag(source_image,
    ...
    updated_init_code, text_embeddings, target_embeddings = drag_diffusion_update(model, init_code,
        None, t, handle_points, target_points, mask, optimize_text, args)
    ...
    if optimize_text:
        gen_image = model(
            prompt=args.prompt,
            encoder_hidden_states=torch.cat([text_embeddings, target_embeddings], dim=0),
            batch_size=2,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--prefix', type=str, default="DEFAULT", help='prefix of the result directory')
