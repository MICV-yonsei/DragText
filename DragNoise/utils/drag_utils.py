...
def drag_diffusion_update(model,
    ...
    with torch.no_grad():
        unet_output, F0 , h_feature = model.forward_unet_features(init_code, t, encoder_hidden_states=target_emb,
    ...
    for step_idx in range(args.n_pix_step):
        ...
        with torch.autocast(device_type='cuda', dtype=torch.float16):
            unet_output, F1, h_feature = model.forward_unet_features(init_code, t, encoder_hidden_states=target_emb,  h_feature=h_feature,
                layer_idx=args.unet_feature_idx, interp_res_h=args.sup_res_h, interp_res_w=args.sup_res_w)
            ...
            for i in range(len(handle_points)):
                ...
                if optimize_text:
                    f1_cur_patch = f1[:,:,int(pi[0])-args.r_m:int(pi[0])+args.r_m+1, int(pi[1])-args.r_m:int(pi[1])+args.r_m+1].detach()
                    f1_next_patch = interpolate_feature_patch(f1, pi[0] + di[0], pi[1] + di[1], args.r_m)
                    loss_text += ((2*args.r_m+1)**2)*F.l1_loss(f1_cur_patch, f1_next_patch)
                    if args.text_mask:
    ...
    return init_code, h_feature, h_features, text_emb, target_emb.detach().requires_grad_(False)
