    ...
    for target_root in args.eval_root:
        ...
        for cat in all_category:
            ...
            for file_name in os.listdir(os.path.join(original_img_root, cat)):
                ...
                print(f"[{cat}] {file_name} - lpips: {cur_lpips.item()}, clip_sim: {cur_clip_sim.item()}")
                torch.cuda.empty_cache()
            category_lpips[cat] = np.mean(cat_lpips)
            category_clip_sim[cat] = np.mean(cat_clip_sim)
        print(target_root)
        for cat in all_category:
            print(f"Category [{cat}] - avg lpips: {category_lpips[cat]}, avg clip sim: {category_clip_sim[cat]}")
