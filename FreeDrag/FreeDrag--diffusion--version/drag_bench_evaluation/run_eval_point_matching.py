        ...
        for cat in all_category:
            ...
            for file_name in os.listdir(os.path.join(original_img_root, cat)):
                ...
                for pt_idx in range(len(handle_points)):
                    ...
                    file_dist.append(dist)
                    cat_dist.append(dist)
                    all_dist.append(dist)
                print(f"[{cat}] {file_name} - mean distance: {torch.tensor(file_dist).mean().item()}")
                torch.cuda.empty_cache()
            category_mean_dist[cat] = torch.tensor(cat_dist).mean().item()
        for cat, mean_dist in category_mean_dist.items():
            print(f"Category [{cat}] mean distance: {mean_dist}")
