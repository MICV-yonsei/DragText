...
def get_current_target(sign_points, current_target,target_point,L,feature_map,max_distance,template_feature,
                       loss_initial,loss_end,position_local,threshold_l):
     ltype = torch.tensor([False] * target_point.shape[0], dtype=torch.bool).to(current_target.device)
     for k in range(target_point.shape[0]):
         if sign_points[k] ==0: # sign_points ==0 means the remains distance to target point is larger than the preset threshold
            current_target[k,:], ltype[k] = get_each_point(current_target[k,:],target_point[k,:],\
                                L,feature_map,max_distance,template_feature[k],loss_initial[k], loss_end[k],position_local,threshold_l)
     return current_target, ltype
...
def freedrag_update(model,
    ...
    # prepare target text embedding to update
    if optimize_text:
        target_emb = copy.deepcopy(text_emb)
        target_emb.requires_grad_(True)
        text_optimizer = torch.optim.Adam([target_emb], lr=args.text_lr)
    else:
        target_emb = text_emb
    # the init output feature of unet
    with torch.no_grad():
        unet_output, F0 = model.forward_unet_features(init_code, t, encoder_hidden_states=target_emb,
    ...
    while True:  
        ...
        current_targets, ltype = get_current_target(sign_points, current_targets,target_points,l_expected,current_F, 
        ...
        for step in range(5):          
            ...
            with torch.autocast(device_type='cuda', dtype=torch.float16):
                unet_output, F1 = model.forward_unet_features(init_code, t, encoder_hidden_states=target_emb,
                layer_idx=args.unet_feature_idx, interp_res_h=args.sup_res_h, interp_res_w=args.sup_res_w)
                ...
                if optimize_text:
                    current_feature_clone = [t.clone() for t in current_feature]
                    F_template_clone = [t.clone() for t in F_template]
                    loss_text_supervised = torch.zeros(point_pairs_number).to(device)
                    for k in range(point_pairs_number):
                        loss_text_supervised[k] = Loss_l1(current_feature_clone[k], F_template_clone[k])
                    loss_text = loss_text_supervised.sum()                
                    if args.text_mask:
                        loss_text += args.text_lam * (text_emb[:,:args.pad_idx,:]-target_emb[:,:args.pad_idx,:]).abs().sum()
            ...
            if loss_supervised.max()<threshold_l and optimize_text == False:
                break
            elif loss_supervised.max()<threshold_l and torch.all(ltype == False):
                break
            elif loss_supervised.max()<threshold_l:
        ...
        with torch.no_grad():
            with torch.autocast(device_type='cuda', dtype=torch.float16):
                _, F1 = model.forward_unet_features(init_code, t, encoder_hidden_states=target_emb,
                layer_idx=args.unet_feature_idx, interp_res_h=args.sup_res_h, interp_res_w=args.sup_res_w)
                # x_prev_updated,_ = model.step(unet_output, t, init_code)
    ...
    return init_code, text_emb, target_emb.detach().requires_grad_(False)
