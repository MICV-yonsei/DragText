...
def create_dragtext_parameters_ui():
    with gr.Tab("DragText Config"):
        with gr.Row():
            optimize_text = gr.Checkbox(value=True, label="optimize text", info="whether to optimize text")
            text_lr = gr.Number(value=0.004, label="text lr", info="learning rate for text optimization")
            text_mask = gr.Checkbox(value=True, label="text regularization mask", info="whether to use text mask to regularize text optimization")
            text_lam = gr.Number(value=0.1, label="text lam", info="regularization strength on text areas")
    
    return text_lam, text_lr, optimize_text, text_mask
...
def attach_run_button_event(run_button, original_image, input_image, mask, prompt,
                            selected_points, inversion_strength, lam, text_lam, latent_lr, text_lr, 
                            model_path, vae_path, lora_path, optimize_text, text_mask,
    ...
    run_button.click(
        ...
        [original_image, input_image, mask, prompt, selected_points,
         inversion_strength, lam, text_lam, latent_lr, text_lr, model_path, vae_path,
         lora_path, optimize_text, text_mask, drag_end_step, drag_per_step, r1, r2, d,
...
def main():
    with gr.Blocks() as demo:
        ...
        text_lam, text_lr, optimize_text, text_mask = create_dragtext_parameters_ui()
        ...
        attach_run_button_event(run_button, original_image, input_image, mask, prompt, selected_points,
                                inversion_strength, lam, text_lam, latent_lr, text_lr, model_path, vae_path, lora_path, optimize_text, text_mask,
